...
public class ListTextureVideoView extends ScalableTextureView
    ...
    private ExecutorService pool = Executors.newCachedThreadPool();//用来执行Callable的线程池，把isPlaying放到别的线程中执行，防止anr
    ...
    public ListTextureVideoView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public ListTextureVideoView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//            am.abandonAudioFocus(null);
    ...
    protected void onDetachedFromWindow() {
        ...
        stop();
        if (mMediaPlayerCallback != null) {
            mMediaPlayerCallback.onDetached();
        }
    ...
    public boolean isPlaying() {
        try {
            Future future = pool.submit(new MyIsPlayingCallable());
            boolean isPlaying = (boolean) future.get();
...
//        setVisibility(VISIBLE);
    ...
    class MyIsPlayingCallable implements Callable {
